    ...
    return (
        <div>
            ...
            <div className="form-check">
                <input className="form-check-input input" type="radio" name={name} id={`${name}${index}`}  onClick={()=>{
                    setPageNumber(1);
                    task(items);
                    }}/>
                <label className="btn btn-outline-primary label" htmlFor={`${name}${index}`}>{items}</label>
            </div>
        </div>
    )
}
